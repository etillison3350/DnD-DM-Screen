 ...
 * example, consider these types: <pre>   {@code
 ...
 * this drawing a rectangle or a diamond? <pre>   {@code
 *   {
 *     "bottomShape": {
 *       "width": 10,
 *       "height": 5,
 ...
 *       "y": 1
 *     }
 *   }}</pre> This class addresses this problem by adding type information to the serialized JSON
 * and honoring that type information when the JSON is deserialized: <pre>   {@code
 *   {
 *     "bottomShape": {
 *       "type": "Diamond",
 *       "width": 10,
 ...
 * name, {@code "type"} will be used. <pre>   {@code
 *   RuntimeTypeAdapterFactory<Shape> shapeAdapterFactory
 *       = RuntimeTypeAdapterFactory.of(Shape.class, "type");
 * }</pre> Next register all of your subtypes. Every subtype must be explicitly registered. This
 * protects your application from injection attacks. If you don't supply an explicit type label, the
 * type's simple name will be used. <pre>   {@code
 *   shapeAdapter.registerSubtype(Rectangle.class, "Rectangle");
 *   shapeAdapter.registerSubtype(Circle.class, "Circle");
 *   shapeAdapter.registerSubtype(Diamond.class, "Diamond");
 * }</pre> Finally, register the type adapter factory in your application's GSON builder:
 * <pre>   {@code
 *   Gson gson = new GsonBuilder()
 *       .registerTypeAdapterFactory(shapeAdapterFactory)
 *       .create();
 * }</pre> Like {@code GsonBuilder}, this API supports chaining: <pre>   {@code

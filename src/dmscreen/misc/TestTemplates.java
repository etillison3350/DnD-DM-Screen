...
public class TestTemplates {
	...
	public static final FeatureTemplate keenSenses = new FeatureTemplate("Keen Sense", Arrays.asList(new TemplateField("shortName", "General Name", FieldType.STRING), new TemplateField("senses", FieldType.LIST, FieldType.STRING, "Hearing", "Sight", "Smell")), "Keen ${senses.length == 1 ? senses[0] : senses.length == 3 || senses.length == 0 ? 'Senses' : senses[0] + ' and ' + senses[1]}", "",
			"${Util.sentenceCase(shortName) + ' has advantage on Wisdom (Perception) checks that rely on ' + (senses.length == 0 ? 'senses' : Util.andJoin('or', senses).toLowerCase()) + '.'}");
	public static final FeatureTemplate basicFeature = new FeatureTemplate("Basic Feature", Arrays.asList(new TemplateField("name", FieldType.STRING), new TemplateField("note", "Note (Optional)", FieldType.STRING), new TemplateField("description", FieldType.STRING)), "${name}", "${note}", "${description}");
	public static final ActionTemplate basicAction = new ActionTemplate("Basic Action", Arrays.asList(new TemplateField("name", FieldType.STRING), new TemplateField("note", "Note (Optional)", FieldType.STRING), new TemplateField("description", FieldType.STRING)), "${name}", "${note}", "${description}");

	public static void main(final String... args) {
		try {
			Files.write(Paths.get("resources/source/templates.json"), Data.GSON.toJson(Stream.of(innate, spellcasting, basicAttack, keenSenses, basicFeature).toArray(size -> new Template[size])).getBytes());
